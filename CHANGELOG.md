...
### Added
- Search field in the panel, between the toolbar and the notes list, to filter the notes currently listed. Plain words match against both the note title and its full body text; terms starting with `#` match against the note's tags by substring, so `#proj` matches `#project`. Prefix any word or tag with `-` to exclude notes matching it, for example `-draft` or `-#archive`. An icon next to the field opens a dropdown to choose how multiple terms combine - "Match ANY words" (the default) lists notes matching any term, "Match ALL words" requires every term; exclusions always apply regardless of that choice. A query of only exclusions narrows the existing list rather than emptying it. Content matching reads through Obsidian's own file cache and settles shortly after you stop typing
...
- Toolbar button toggling between the tag list view and a title list view. The title view drops the tag groupings and lists every matching note once in a single flat list, with the number of tags each note matched appended like `(3)`. The button shows a hash icon while grouped by tag and a file icon while listing by title, and the tag group sort control is disabled in the title view since there are no groups to order
- "Show notes in all matching tag groups" setting. A note matching several tags is normally repeated under each of them; turning this off lists each note once only, under the first of its tags alphabetically, with the number of tags it matched appended like `(3)`. The home group is deliberately fixed rather than following the displayed order, so changing the tag group sort reorders the groups without moving notes between them. The count uses the theme's normal text colour rather than the configurable title colour, so it reads as metadata
