...
- "Show notes in all matching tag groups" setting. A note matching several tags is normally repeated under each of them; turning this off lists each note once only, under its first matching tag group, with the number of tags it matched appended like `(3)`. The count uses the theme's normal text colour rather than the configurable title colour, so it reads as metadata
